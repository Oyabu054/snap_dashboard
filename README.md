...
内部では本番向けWSGIサーバー(`waitress`)でアプリを起動します。
...
- アプリは本番向けWSGIサーバー(`waitress`)で動作します。閲覧5台程度・
  リアルタイム更新は30秒間隔ポーリングという軽い負荷を想定した`threads=8`で
  起動しています。同時アクセスが増えて動作が不安定になるようであれば、
  `app.py`の`serve(..., threads=8)`の値を調整してください
