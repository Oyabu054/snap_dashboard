...
- **Git**: `master`ブランチ、作業ツリークリーン。ローカルの最新コミットは
  `fb9d9b8`(引き継ぎ資料更新)。**ただし`origin/master`へは未push**(ローカルが4コミット
  進んでいる状態: `ce890e1`waitress移行 → `687bef5`UI仕様変更 → `7570911`LOBB修正 →
  `fb9d9b8`引き継ぎ資料更新)。pushのタイミングはユーザーに確認すること
  リポジトリ: `git@github.com:Oyabu054/snap_dashboard.git`
- **本番向けサーバー(waitress)への移行は実装完了・コミット済み**(2026-07-22)。`app.py`を
...
- **UI仕様変更一式が実装完了・コミット済み・ユーザー実機で好評価**(2026-07-22、
  「順調に出来た」とのフィードバック)。クイック選択廃止・トレンド凡例のタイトル横移動・
  サブタイトル/マップタイトル削除・マップ/トレンドのflex比率変更(トレンドを元の2/3の
  高さに圧縮しマップを拡大)・X軸目盛りのdtick/tick0明示指定によるズレ対策・
  マップ⇔トレンドのドラッグズーム双方向追従、の7点(+高さ比率の追加調整1点)。
  詳細・技術的な注意点は下記「UI仕様変更(2026-07-22)」参照
- **LOBB位置がマップに表示されない不具合を修正・コミット済み**(2026-07-22)。原因は
  `pi_client.py`の`ATTR_LOBB_POSITION`が`"LOBB位置"`(スペースなし)と定義されていたが、
  実際のPI側属性名は`"LOBB 位置"`(半角スペースあり)だったため常に0件を返していたこと。
  このWSL環境から実PIサーバーへ接続し、属性名の実値照会・修正後のデータ取得(過去14日で
  21件)まで確認済み。ユーザー実機でも「問題なかった」と確認済み
...
- **次にやるべきこと**: (1)`origin/master`へのpush可否をユーザーに確認、(2)waitress移行の
  実機確認(ビルド・起動・複数PC接続)、(3)UI変更の実機ブラウザでの最終確認は完了済みだが
  追加で気になる点がないか確認(サンドボックス環境ではヘッドレスブラウザが動かせず
  こちら側では視覚確認できていない、下記参照)
...
1. **ローカルの4コミット(`ce890e1`〜`fb9d9b8`)を`origin/master`へpushするか**、
   次のセッションでユーザーに確認すること(コミット自体は完了済み)
2. **waitress移行の実機確認**: `pyinstaller SnapMonitor.spec`→`dist/SnapMonitor.exe`起動→
   ブラウザ自動オープン→PI接続成功→複数PCからのアクセスを再確認してもらう
   (UI変更・LOBB修正は実機ブラウザで確認済み・「問題なかった」との回答済み)
