...
- **Git**: `master`ブランチ、直近コミット`431ff76`まではpush済み。このセッションでの
  タイムゾーン修正・CST回転数等の追加分はまだコミット・push前(要ユーザー確認)。
  リポジトリ: `git@github.com:Oyabu054/snap_dashboard.git`
- **exe化は完了・実機確認済み**: `pyinstaller SnapMonitor.spec`でのビルド・起動・PI接続を
  ユーザー実機で確認済み(2026-07-22)。複数PC(最大5台程度)からの閲覧もコード変更不要で
  対応可能(README「8.」参照)
- **今回のセッションで追加した機能**: PI AFにCST回転数・厚み・絶対真空圧の3属性が追加され、
  時間帯別トレンドグラフに別軸で重ね描画する機能を実装(左パネルの「グラフ表示要素」
  チェックボックスでON/OFF可能)。リアルタイム更新をデフォルトON・5分間隔に変更、
  初期表示範囲を24時間→3日間に変更。詳細は下記「トレンドグラフの重ね描画系列」
  「リアルタイム監視について」参照
- **重要: タイムゾーンのずれを原因確定・修正済み(2026-07-22)**: 実機で「直近のスナップが
  取得できない」と報告があり調査した結果、フロントエンドが日時をUTCに変換してから送信していた
  ため、SQLクエリが実際には9時間早い範囲をPIに問い合わせていたことが判明。`dashboard.js`の
  `applyFilter()`を修正済み(下記「既知のリスク」参照)。**この修正はまだコミットしていない**
- **次にやるべきこと**: (1) タイムゾーン修正後、実機で直近のスナップが正しく表示されるか
  再確認してもらう。(2) 問題なければコミット・push。(3) 既知リスクの残り(DefectType列挙値・
  duration_unit)の実データ確認
