...
## 現在地点(2026-07-23時点、次のチャットで最初に読む部分)

- **Git**: `master`ブランチ、作業ツリークリーン。ローカルの最新コミットは配色見直し関連
  (下記参照)。**`origin/master`へは未push**(ローカルが1コミット進んでいる状態)。
  pushのタイミングはユーザーに確認すること
  リポジトリ: `git@github.com:Oyabu054/snap_dashboard.git`
- **配色の見直し・マップ凡例・フィルター順序変更を実装完了・コミット済み**(2026-07-23)。
  ユーザーから「ドリップ/Ｒ／Ｋ/LOBBの色が似ている」「ドロス/疵/失透の色が似ている」
  「マップ右上に表示中の欠点種類の凡例が欲しい」「フィルター順序をリーム/ドリップ/
  ドロス/LOBB/失透/汚れ/疵/砂利線/Ｒ／Ｋにしたい」との指摘、その後「リームはピンク、
  失透は緑系で」との追加指定を受け対応。欠点種類の色・表示順を名前キーの固定テーブル
  `DEFECT_LEGEND_ORDER`で管理する方式に変更し、`dataviz`スキルの`validate_palette.js`で
  検証済みの色を割り当てた。詳細は下記「配色の見直し・マップ凡例・フィルター順序変更
  (2026-07-23)」参照。**サンドボックスでは視覚確認できていないため実機確認が必要**
...
- **次にやるべきこと**: (1)`origin/master`へのpush可否をユーザーに確認、(2)今回の
  配色見直し・マップ凡例・フィルター順序変更をユーザー実機で最終確認(サンドボックスでは
  視覚確認できていない)、(3)waitress移行の実機確認(ビルド・起動・複数PC接続。
  ビルド未実施のままなら継続確認)
...
| 領域 | 内容 |
|---|---|
| 左 | フィルターパネル: 開始/終了日時、表示範囲絞り込みスクロールバー(デュアルハンドル、データ再取得なしで両チャートをズーム。マップ/トレンドのドラッグズームとも双方向連動)、スナップ種類チェックボックス(全選択/全解除ボタン付き。**表示順はリーム/ドリップ/ドロス/LOBB/失透/汚れ/疵/砂利線/Ｒ／Ｋの固定順、2026-07-23変更**)、適用ボタン、下部に「ストリエboxリンク」(外部Boxフォルダへのリンク、後述)。**クイック選択(1h/8h/24h/7d)は2026-07-22に廃止済み** |
| 中央上 | スナップマップ: X=日時(右が最新)、Y=位置0〜210(**上=左側=大きい値、下=右側**)。欠点は発生時刻→+継続時間の**横線分**で表示(マーカーサイズでの表現は却下済み)。ホバーで種類/発生時刻/位置/継続時間のツールチップ表示。Gross/Net幅の凡例は非表示(ユーザー指示で削除済み)。**タイトル見出しは2026-07-22に廃止し、その分を含めチャート領域を拡大**(左/右の目安ラベルはチャート内に残置)。ドラッグで矩形選択ズーム可能、トレンド側と双方向で範囲が追従する。**右上に表示中の欠点種類・LOBB・失透の色見本を注釈として重ね表示(2026-07-23追加、`#mapLegend`)** |
...
## 配色の見直し・マップ凡例・フィルター順序変更(2026-07-23)

ユーザーから、実機スクリーンショットに基づき3点の指摘を受け対応した:
「ドリップ・Ｒ／Ｋ・LOBBの色味が似ている(いずれも青系)」
「ドロス・疵・失透の色味も似ている(いずれも緑系)」
「マップ右上の空きスペースに、現在表示中の欠点種類と色の注釈が欲しい」
「左パネルのフィルター順序をリーム/ドリップ/ドロス/LOBB/失透/汚れ/疵/砂利線/Ｒ／Ｋにしたい」

### 配色の再設計

- 従来は`PALETTE[i % 8]`のように、APIが返す欠点種類の**配列インデックス**に紐付けて
  色を割り当てていた(欠点種類の並び順が変わると既存の種類の色も変わってしまう設計上の
  弱点があった)。LOBB(`#1e3a8a`濃紺)・失透(`#65a30d`オリーブ)は既存8色パレットとは
  別に、ad hocに決めた色だった
- `dataviz`スキルの`scripts/validate_palette.js`(`--pairs all`、マップ上に複数系列が
  同時に見える散布図的な表示のため全ペア総当たりで検証)で現状の9色
  (欠点7種+LOBB+失透)を検証したところ、**ハードFAIL**だった:
  `#1e3a8a`(LOBB)↔`#4a3aa7`(Ｒ／Ｋ)のnormal-vision ΔE 7.8(floor 15を下回り、
  色覚正常でも判別困難)、`#eb6834`(砂利線)↔`#008300`(ドロス)のCVD ΔE 3.2など。
  ユーザーの指摘は数値的にも裏付けられた
- `dataviz`スキルの`palette.md`には「散布図・マップ等の全ペア総当たり(`--pairs all`)
  形式では、8色パレットのうち先頭3色しか全ペアで安全な組み合わせを作れない
  (数学的な制約で、8色全体をどう並べ替えても全ペア合格は不可能)」と明記されている。
  9系列を同時に見せる以上、色相だけで完全にCVD安全な組み合わせを作ることは原理的に
  不可能なため、以下の現実的な方針で対応した:
  1. 実際の欠点7種類には検証済みの8色パレットのうち7色(青/橙/アクア/黄/マゼンタ/緑/紫、
     8色目の赤`#e34948`は未使用のため予備に残す)を割り当てる
  2. LOBB・失透の色を、上記7色すべてから可能な限り離れた色になるようOKLCH色空間を
     総当たり探索するスクリプト(スクラッチパッドの`palette/search5.mjs`、
     セッション限りの使い捨てスクリプトのためリポジトリには残していない)で選定。
     LOBB→`#910058`(濃い紅色)に変更(この色は既存7色のどれとの組み合わせでも
     CVD ΔE 14以上・normal-vision ΔE 28以上を確保)
  3. 残った不合格ペア(欠点7種類**同士**の組み合わせ、後述)は既存の検証済み8色パレット
     自体が内包する既知のトレードオフ(標準パレットであり変更のスコープ外)。今回は変更していない
- **2026-07-23、同日中にユーザーから色の再指定あり(「リームはピンク、失透は緑系で」)**:
  上記の初回対応では失透をシアン`#00beff`にしていたが、ユーザーの追加要望を受けて
  7色の割り当てを組み替えた。最終的な割り当て(`DEFECT_LEGEND_ORDER`):
  リーム→マゼンタ`#e87ba4`(ピンク)、ドリップ→橙`#eb6834`、ドロス→アクア`#1baf7a`、
  LOBB→濃い紅色`#910058`、失透→緑`#008300`、汚れ→黄`#eda100`、疵→青`#2a78d6`、
  砂利線→シアン`#00beff`、Ｒ／Ｋ→紫`#4a3aa7`。
  懸念だった「ドロス(アクア)↔失透(緑)」はCVD ΔE 15.2・normal-vision ΔE 15.6で
  ともに基準(target 8.0・floor 15.0)をクリア(検証済み)。残る不合格ペアは
  `失透`↔`ドリップ`のCVD(ΔE3.2、緑↔橙)と`リーム`↔`ドリップ`のnormal-vision
  (ΔE12.9、ピンク↔橙)で、いずれも8色パレット内の既知のトレードオフ(今回の
  ユーザー指摘の対象外)。ユーザーからは「修正後コミットしてほしい」と依頼され
  コミット済み(下記「現在地点」参照)
- 色の割り当ては`dashboard.js`の`DEFECT_LEGEND_ORDER`という**名前をキーにした固定配列**に
  変更した(欠点種類の文字列→色・表示順を1つのテーブルで管理)。APIが返す欠点種類の
  順序が変わっても、同じ名前の欠点は常に同じ色になる(dataviz skillの「色はentityに
  従う、出現順に従わない」という原則に対応)。`DEFECT_LEGEND_ORDER`に載っていない
  未知の欠点種類(PI側で新種が追加された場合)は`FALLBACK_PALETTE`から色を割り当てて
  末尾に追加する(表示が消えることはない)

### マップ右上の凡例

`updateMapLegend(types, showLobb, showDevitrification)`(`dashboard.js`)を新設し、
`applyFilter()`から呼ぶ。現在チェックが入っている欠点種類・LOBB・失透だけを
`DEFECT_LEGEND_ORDER`の固定順で抽出し、`#mapLegend`(`.chart-block--map`内、
`position:absolute`でマップ右上に重ねて配置)に色スウォッチ付きで表示する。
`pointer-events:none`にしているため、凡例の上からでもマップのドラッグズーム操作は
そのまま行える。

### フィルター順序

`renderDefectTypeCheckboxes(defectTypes)`(`dashboard.js`)を新設し、
`DEFECT_LEGEND_ORDER`の固定順でチェックボックスを描画するように変更
(リーム/ドリップ/ドロス/LOBB/失透/汚れ/疵/砂利線/Ｒ／Ｋの順、ユーザー指定通り)。
実際にAPIが返した欠点種類だけを表示する点(未出現の種類は表示しない)は従来通り。
LOBB・失透は常に表示(従来通り、末尾ではなく指定位置に組み込み)。

**検証方法**: `dataviz`スキルの`validate_palette.js`で新9色を`--pairs all`で再検証し、
LOBB・失透が関与する全ペアがtarget(ΔE8.0)・floor(ΔE15.0)を上回ることを数値で確認。
Node上に`document`のスタブを用意して`dashboard.js`を直接評価し、
`renderDefectTypeCheckboxes()`の出力順・色が意図通りであることを確認。
既存pytest(74件)全パス・`node --check`でJS構文エラーなしを確認。
**サンドボックス環境ではブラウザ描画の確認ができないため、実際の見た目(凡例の位置・
可読性、色の見分けやすさ)はユーザー実機での確認が必要**。
...
- スナップ種類・LOBB・失透の配色と表示順は`DEFECT_LEGEND_ORDER`(`dashboard.js`)という
  名前キーの固定テーブルで管理する(2026-07-23改訂。詳細は「配色の見直し(2026-07-23)」参照)。
